...
int main()
{
   ...
   do
   {
      ...
      glDrawArrays(GL_POINTS, NUMBER_OF_TRIANGLES*3, 8);
...
void createVBO(GLuint &vertexbuffer, GLuint &colorbuffer, GLuint &pointbuffer, GLuint &VertexArrayID)
{
   ...
   for(unsigned i=0; i<NUMBER_OF_TRIANGLES; ++i)
   {
      ...
      do
      {
         ...
         temp_area = triangleArea(point1.x, point1.y, point2.x, point2.y, point3.x, point3.y);

      } while(!((temp_area > 0.1) && (temp_area < 0.2)));
